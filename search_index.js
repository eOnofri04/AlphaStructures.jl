...

{
    ...
    "category": "section",
    "text": "We can define a Delunay Triangulation mathcal D over the set S and, since the general position condition is satisfied by hypotesis, it follows that the thetrahedra decomposition of the convex hull it\'s unique.For 0 leq k leq 3 we can define F_k as the k-simplices set of mathcal D. It is pretty clear that if a simplex sigma in F_k belongs to mathcal T then it will also belong to mathcal D. It could however be proven that for all the k-simplices sigma of F_k there exists a baralpha such that sigma belongs to F_k baralpha (namely the baralpha-shape k-simplices):	F_k = bigcup_0 leq alpha leq infty F_k alpha qquad forall k in 0 1 2 3We will call baralpha as Charateristic alpha of the simplex sigma and we will denote it like alpha_sigma. It is usefull then to sort the simplices according to their alpha-charateristic.We then obtain a sorted finite set    Alpha_S = alpha_sigma_i mid sigma_i in F_k exists k = 1 2 3However in order to determine Alpha_S there is a better path than evaluate each single alpha-charateristic. This strategy relies on the dual contruction of Delaunay Triangulation: Voronoy Diagrams.Let mathcal V be the Voronoy Decomposition of S. For all site t in S we will denote V_t the Voronoy cell of t."
},

{
    "location": "theory.html#d2-1",
    "page": "Teorethical Description",
    "title": "d=2",
    "category": "section",
    "text": "alpha_sigma could be determined as follows:if sigma in F_0 then alpha_sigma = 0\nif sigma in F_2 then alpha_sigma is the radius of the circumcircle of the triangle\nif sigma = overlinest in F_1 then two cases arise:\nif overlinest intersect the Voronoy segment between V_s and V_t (namely overlinest cap V_s cap V_t ne emptyset) then sigma = sigma2\nif overlinest do not intersect the Voronoy segment then there exists u in S such that the angle shat ut  pi2. Then alpha_sigma is the radius of the circumcircle of the triangle sut.It is trivial that 0-cells always are in the alpha-Complex.Moreover it is also pretty clear that if a 2-cell belongs to the alpha-complex then also its 1-cells are part of it."
},

{
    "location": "theory.html#d-3-1",
    "page": "Teorethical Description",
    "title": "d = 3",
    "category": "section",
